...
ChooseFileNumber = [2]; % what files to run
ChooseTetrodeNumber=[1 2 3 4 5 6 7 8]; % what Tetrodes to run [1 2 3 4 5 6 7 8]
...
k = find(X);
histo = X(k);
edges = [-0.02:0.0005:0.02]; 
histogram(histo,edges,'FaceColor','k','FaceAlpha',1)

ax = gca;
ax.XTick = -0.02:0.005:0.02;
ax.XLim = [-0.02 0.02];
MaxLim=(round(size(cell,2)/80));
if MaxLim > 100;
    MaxLim =100;
end
if MaxLim < 30;
