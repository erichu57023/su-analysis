function spikes = ss_energy_parallel(spikes)
...
%Parallelized by BI 2015-08-20

% slicedClustID:
linInd=1;
% ind=zeros(numClusts^2,2);
for clust1 = 1:numclusts
    for clust2 = clust1:numclusts   % clust2 starts at clust1 so we get intra- too 
        ind(linInd,:)=[clust1 clust2];
        linInd=linInd+1;
    end
end


%Instead of nesting for loops, ceate 1 for loop:
%Parallel processor intensive part:
parfor i=1:size(ind,1)
    dists = pairdist(waves(pts{ind(i,1)},:), waves(pts{ind(i,2)},:));
    interface_energy_temp(i) = sum(exp(-dists(:)/scale));
end

%Reorganize data in identical way:
linInd=1;
interface_energy=zeros(numclusts);
for clust1 = 1:numclusts
   for clust2 = clust1:numclusts         
        interface_energy(clust1,clust2)=interface_energy_temp(linInd);
        linInd=linInd+1;
   end
end
