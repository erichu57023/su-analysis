function splitmerge_tool_PS( spikes, show_clusters, h, varname)
...
%
   warning('off' , 'all');
    ...
    set(h,'WindowScrollWheelFcn',@WindowScrollWheelFcn_mergetool);% ADDED BY PS - 131009
    ...
    % ADDED BY PS - 131009
    uipushtool(th,'CData',data.ims.im_scale,'ClickedCallBack',{@adjustYlim,h},'TooltipString','Adjust ylim of all panels to selected(!) one','Separator','on');
    uipushtool(th,'CData',data.ims.im_scatter3d,'ClickedCallBack',{@RemoveArtifacts,h},'TooltipString','Remove artifacts','Separator','off');
    uipushtool(th,'CData',data.ims.im_sep,'ClickedCallBack',{@makeSeparationAnalysis2figs,h},'TooltipString','Compare selected cluster *across* figures!');
    % ADDED BY PS - 132009
    
    % save spikes data to figure
    % ADDED BY PS - 141124
    spikes.params.display.label_categories = {'in process', 'good unit', 'multi-unit', 'garbage', 'needs outlier removal', 'medium unit', 'bad unit','low n','corr','unstable'};
    spikes.params.display.label_colors = [ .7 .7 .7; .3 .8 .3; .3 .3 .8;  .8 .3 .3; .7 .7 .3;.3 .6 .3;.3 .4 .3; .5 .4 .5; .3 .3 .3; .5 .5 .5];
    % ADDED BY PS - 141124
    ...
    % ADDED BY PS - 131101
    try
    set(h,'Name',['Merge Tool - ',spikes.params.display.dataset],'NumberTitle','off') % PS - 131101
    catch
...
function saveSpikesToFile(varargin)
    ...
    if ~isequal(FileName,0)
        ...
        % Appended by PS -140320
        try
            attributes = evalin('base','attributes');
            save([PathName FileName],'spikes','attributes','-v7.3');
        catch
        save([PathName FileName],'spikes','-v7.3'); % originally only this line (added -v7.3 for larger files)
        disp('No attributes file found! Saving only spikes...');
        end
        % Appended by PS -140320
...
function makeShowClust(varargin)
      ...
      if isempty(selected)
                show_clusters_PS_v0(figdata.spikes);
      else
                show_clusters_PS_v0(figdata.spikes,selected);
      end
end
...
function makeSeparationAnalysis(varargin)
      ...
      selected = figdata.selected;
      if isempty(selected)
          compare_clusters_PS_v0(figdata.spikes);
      else
          compare_clusters_PS_v0(figdata.spikes,selected);
      end
end

% make instance of compare_clusters
function makeSeparationAnalysis2figs(varargin)
      figdata1 = get(varargin{end},'UserData');
      selected1 = figdata1.selected;
      figdata2 = get(varargin{end}+1,'UserData');
      selected2 = figdata2.selected;
      selected = [selected1 selected2];
      if isempty(selected)
%           compare_clusters_PS_v0(figdata.spikes);
      else
          compare_clusters_PS_2figs_v0(figdata1.spikes,figdata2.spikes,selected);
      end
end
...
function makePlotFeatures(varargin)
      ...
      % PS: 3=colormode clusters, 0=dont show outliers
      plot_features_PS(figdata.spikes,selected,3,0)
end
...
% ADDED BY PS - 131009 ----------------------------------------------------
function adjustYlim(varargin)

    h = varargin{end};
    axid=findobj(h,'tag','waveforms');
    
    try
        selcl = get(findobj(h,'backgroundcolor','w'),'children');
        seldat = get(selcl,'UserData');
        ymax = max(max(max(seldat.waveforms)));
        ymin = min(min(min(seldat.waveforms)));
        set(axid,'ylim',[ymin+ymin*.2 ymax+ymax*.2]);
    
    catch
        disp('Select 1(!) template cluster first!');
    end

end

function RemoveArtifacts(varargin)
h = varargin{end};
figdata = get(h,'UserData');
hardthr = 300; % fixed! in uV

try
clus_id = figdata.selected;

for kk=1:length(clus_id)

sp_ids = figdata.spikes.assigns==clus_id(kk);
out_id = false(1,length(sp_ids));

sp_tmp = figdata.spikes.waveforms(sp_ids,:,:);

sp_mean = squeeze(median(sp_tmp,1));
sp_sd = squeeze(std(sp_tmp,1));

artid = false(1,size(sp_tmp,1));

p1 = max(max(sp_tmp,[],2),[],3);
p2 = min(min(sp_tmp,[],2),[],3);

tmpid = unique([find(p1>hardthr); find(p2<-hardthr)]);
artid(tmpid) = true;

% f0 = figure;
% hold on
% for t=1:length(tmpid)
%     for n=1:size(sp_tmp,3)
%     plot(sp_tmp(tmpid(t),:,n));
%     end
% end

disp(['Cluster ',num2str(clus_id(kk)),':']);

if ~isempty(find(artid==true))
a = find(sp_ids==true);
out_id(a(find(artid==true))) = true;

figdata.spikes = remove_outliers(figdata.spikes, out_id);

disp([num2str(length(find(artid==true))),' artifact(s) detected and tagged at +/- ',num2str(hardthr),'{\muV}']);
else
    disp(['No artifact(s) detected at +/- ',num2str(hardthr),'{\muV}']);
end
disp('-----------------------------------------------------');
disp('-----------------------------------------------------');
remote_update( h, figdata.spikes, clus_id(kk) );

end
catch
    disp('Select at least 1 cluster for artifact removal!');
end
...
function click_panel( varargin )
    ...
    if isequal( get(h,'SelectionType'), 'open' ) % added by PS 140515
        adjustYlim(h);
    end
    % update list
...
function open_split_tool( varargin )
    ...
    clus = varargin{end};
    figdata = get(h,'UserData');
    split_tool_PS( figdata.spikes, clus,[],@remote_update,h ); % Changed by PS 131011
end
