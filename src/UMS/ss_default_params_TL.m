function spikes = ss_default_params_TL(Fs, varargin )
...
%
    ...
    spikes.params.thresh = 3.5;            % for 'auto', set number of standard deviations above background noise
 %  spikes.params.thresh =    [-5.3897 -5.0999 -5.1816 -5.5274]; % for 'manual', set specific thresholds for each channel
    ...
    spikes.params.shadow = 0.66;            % ms, enforced dead region after each spike
    spikes.params.cross_time = 0.5;         % ms, alignment point for peak of waveform
 
    % sorting parameters
    spikes.params.refractory_period = 1.5;  % ms, refractory period (for calculation refractory period violations) %Later changed to 1.5ms - BI
    spikes.params.max_jitter = 0.5;         % ms, width of window used to detect peak after threshold crossing
    spikes.params.agg_cutoff = .25;         %  higher = less aggregation, lower = more aggregation
%     spikes.params.kmeans_clustersize = 20; %  target size for miniclusters
    spikes.params.kmeans_clustersize = 1000;
    ...
    spikes.params.display.show_outliers = 0;      % 0 = hide outliers, 1 = show outliers
    ...
    spikes.params.display.trial_spacing = 0;% .5;           % s, time padding between trials %UPDATED 2016-02-16!!! Unwrapped times are wrong for all previous files unless corrected.
