function spikes = ss_detect_BI( data, spikes )
...
%         thresh2=  ones(size(thresh)) *-200; %Avoid noise (clicks above 200 uV) Doesn't work...BI 7-3-13
...
%         crossings = [crossings find( ((data{j}(1:end-1,k) > thresh2(k))) & ((data{j}(1:end-1,k) > thresh(k)) &  data{j}(2:end,k) <= thresh(k)) )' ];  % %Avoid noise (clicks above 250 uV) BI 10-8-13
        crossings = [crossings find( (data{j}(1:end-1,k) > thresh(k)) &  data{j}(2:end,k) <= thresh(k) )' ];  % ad
